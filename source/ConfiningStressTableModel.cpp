...
auto ConfiningStressTableModel::columnCount(
    const QModelIndex & /* parent */) const -> int {
  return 4;
}

auto ConfiningStressTableModel::headerData(int section,
...
auto ConfiningStressTableModel::data(const QModelIndex &index, int role) const
    -> QVariant {
...
auto ConfiningStressTableModel::setData(const QModelIndex &index,
...
auto ConfiningStressTableModel::flags(const QModelIndex &index) const
    -> Qt::ItemFlags {
  if (index.column() == 3) {
...
void ConfiningStressTableModel::computeStress(int layer) {
  ...
  for (int i = layer; i < _layers.size(); ++i) {
    ...
    double porePressure =
        (depth < _waterTableDepth)
            ? 0
            : (depth - _waterTableDepth) * Units::instance()->waterUntWt();

    double vEffStress = vStress - porePressure;

    double mEffStress =
        vEffStress * (1. + 2. * _layers.at(i)->atRestCoeff) / 3.;
