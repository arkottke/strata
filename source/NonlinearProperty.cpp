...
NonlinearProperty::NonlinearProperty(const QString &name, Type type,
                                     const QVector<double> &strain,
                                     const QVector<double> &property,
                                     QObject *parent)
...
auto NonlinearProperty::name() const -> const QString & { return _name; }
...
auto NonlinearProperty::interp(const double strain) -> double {
  ...
  if (strain < _strain.first()) {
    ...
    if (!_interp)
      initialize();
    d = gsl_interp_eval(_interp, _lnStrain.data(), _varied.data(), log(strain),
                        _acc);
  }
...
auto NonlinearProperty::data(const QModelIndex &index, int role) const
    -> QVariant {
...
auto NonlinearProperty::headerData(int section, Qt::Orientation orientation,
...
void NonlinearProperty::setVaried(const QVector<double> &varied) {
  ...
  // Need to ensure that the strains are constantly increasing. This is required
  // for the GSL interpolation
...
void NonlinearProperty::initialize() {
  if (_lnStrain.size() > 2) {
    ...
    gsl_interp_init(_interp, _lnStrain.data(), _varied.data(),
                    _lnStrain.size());
...
auto operator<<(QDataStream &out, const NonlinearProperty *np)
    -> QDataStream & {
  out << (quint8)1;
