...
auto GeneralPage::createProjectGroupBox() -> QGroupBox * {
  ...
  layout->addWidget(
      new QLabel(
          tr("Note: only changes labels and gravity, no unit conversion.")),
      4, 2);

  // Save motion data
  _saveMotionDataCheckBox =
      new QCheckBox(tr("Save motion data within the input file."));
...
auto GeneralPage::createVariationGroupBox() -> QGroupBox * {
  ...
  _nlPropertiesAreVariedCheckBox =
      new QCheckBox(tr("Vary the nonlinear properties"));
  layout->addRow(_nlPropertiesAreVariedCheckBox);

  auto *label = new QLabel(tr("-- shear-modulus reduction curve\n"
                              "-- damping ratio curve\n"
  ...
  label = new QLabel(tr("-- shear-wave velocity\n"
                        "-- layer thickness\n"
                        "-- depth to bedrock"));
  label->setIndent(indent);
  layout->addRow(label);
...
auto GeneralPage::createDiscretizationGroupBox() -> QGroupBox * {
  ...
  _disableDiscretzationCheckBox =
      new QCheckBox(tr("Disable auto-discretization"));
