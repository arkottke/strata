...
auto SoilTypePage::createBedrockGroupBox() -> QGroupBox * {
  ...
  _varyBedrockDampingCheckBox =
      new QCheckBox(tr("Vary the damping of the bedrock"));
...
auto SoilTypePage::createVariationGroupBox() -> QGroupBox * {
  ...
  _dampingUncertWidget =
      new NonlinearPropertyUncertaintyWidget(tr("Damping:"), layout, this);
  ...
  layout->addWidget(
      new QLabel(tr("G/G_max, Damping Correlation Coefficent (%1):")
                     .arg(QChar(0x03C1))),
      row, 0, 1, 2);
  ...
  // Group box
  _randomizerGroupBox =
      new QGroupBox(tr("Nonlinear Curve Variation Parameters"));
...
auto SoilTypePage::createNlPropTableBox() -> QGroupBox * {
  ...
  _nlPropTableBox->table()->setItemDelegateForColumn(
      0, new OnlyIncreasingDelegate);
...
void SoilTypePage::updateUnits() {
  ...
  _bedrockUntWtSpinBox->setRange(_bedrockUntWtSpinBox->minimum(),
                                 _bedrockUntWtSpinBox->maximum());
}

void SoilTypePage::selectIndex(const QModelIndex &current,
...
void SoilTypePage::updateDampingRequired(bool b) {
  _soilTypeTableBox->setColumnHidden(SoilTypeCatalog::DampingColumn, !b);
}

void SoilTypePage::updateNonlinearPropertiesRequired(bool b) {
  _nonlinearPropsRequired = b;

  _soilTypeTableBox->setColumnHidden(SoilTypeCatalog::ModulusModelColumn, !b);
  _soilTypeTableBox->setColumnHidden(SoilTypeCatalog::DampingModelColumn, !b);
}

void SoilTypePage::updateVariedColumn(bool show) {
  _soilTypeTableBox->setColumnHidden(SoilTypeCatalog::IsVariedColumn, !show);
}
