...
auto SoilTypeCatalog::toHtml() const -> QString {
  ...
  // Need to leave the ordered list open because the bedrock data will be added
  // to it.
...
auto SoilTypeCatalog::setData(const QModelIndex &index, const QVariant &value,
                              int role) -> bool {
  ...
  if (role == Qt::EditRole) {
    ...
    case ModulusModelColumn:
      if (NonlinearProperty *np =
              _nlCatalog->modulusFactory()->duplicateAt(value)) {
    ...
    case DampingModelColumn:
      if (NonlinearProperty *np =
              _nlCatalog->dampingFactory()->duplicateAt(value)) {
...
auto SoilTypeCatalog::headerData(int section, Qt::Orientation orientation,
...
auto SoilTypeCatalog::insertRows(int row, int count, const QModelIndex &parent)
    -> bool {
  if (!count)
    return false;
...
auto SoilTypeCatalog::removeRows(int row, int count, const QModelIndex &parent)
    -> bool {
  if (!count)
    return false;
