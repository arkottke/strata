...
SoilType::SoilType(QObject *parent) : QObject(parent) {
  ...
  _modulusModel = new CustomNonlinearProperty(
      NonlinearProperty::ModulusReduction, false, this);
  _dampingModel =
      new CustomNonlinearProperty(NonlinearProperty::Damping, false, this);
...
auto SoilType::untWt() const -> double { return _untWt; }
...
auto SoilType::damping() const -> double { return _damping; }
...
auto SoilType::name() const -> const QString & { return _name; }
...
auto SoilType::notes() const -> const QString & { return _notes; }
...
auto SoilType::isVaried() const -> bool { return _isVaried; }
...
auto SoilType::saveData() const -> bool { return _saveData; }

auto SoilType::modulusModel() -> NonlinearProperty * { return _modulusModel; }
...
auto SoilType::dampingModel() -> NonlinearProperty * { return _dampingModel; }
...
auto SoilType::meanStress() const -> double { return _meanStress; }
...
auto SoilType::pi() const -> double { return _pi; }
...
auto SoilType::ocr() const -> double { return _ocr; }
...
auto SoilType::freq() const -> double { return _freq; }
...
auto SoilType::nCycles() const -> int { return _nCycles; }
...
auto SoilType::toHtml() const -> QString {
  ...
  if (qobject_cast<DarendeliNonlinearProperty *>(_modulusModel) ||
      qobject_cast<DarendeliNonlinearProperty *>(_dampingModel))
    html += tr("<tr><th>Mean Stress:</th><td>%1 atm</td></tr>"
...
void SoilType::fromJson(const QJsonObject &json) {
  ...
  NonlinearProperty *mnp =
      deriveModel(NonlinearProperty::ModulusReduction, modulusType);
  Q_ASSERT(mnp);
...
auto operator<<(QDataStream &out, const SoilType *st) -> QDataStream & {
  ...
  out << st->_untWt << st->_damping << st->_name << st->_notes << st->_isVaried
      << st->_saveData << st->_meanStress << st->_pi << st->_ocr << st->_freq
      << st->_nCycles << QString(st->_modulusModel->metaObject()->className())
      << st->_modulusModel
...
auto operator>>(QDataStream &in, SoilType *st) -> QDataStream & {
  ...
  in >> st->_untWt >> st->_damping >> st->_name >> st->_notes >>
      st->_isVaried >> st->_saveData >> st->_meanStress >> st->_pi >>
      st->_ocr >> st->_freq >> st->_nCycles;
...
auto SoilType::deriveModel(NonlinearProperty::Type type, QString className)
    -> NonlinearProperty * {
  ...
  if (className == "CustomNonlinearProperty") {
    ...
    np =
        qobject_cast<NonlinearProperty *>(new DarendeliNonlinearProperty(type));
