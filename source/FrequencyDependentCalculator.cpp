...
auto FrequencyDependentCalculator::updateSubLayer(
  ...
  // Update the sublayer with the representative strain -- FIXME strainAvg
  // doesn't appear to be representative
  ...
  if (_useSmoothSpectrum) {
    // Compute the mean frequency and mean strain parameters defined by Kausel
    // and Assimaki (2002)
    double numer = 0;
    double denom = 0;
    double dFreq;
    ...
    for (int i = 1; i < freq.size(); ++i) {
      ...
      numer += dFreq *
               (freq.at(i - 1) * strainFas.at(i - 1) +
                freq.at(i) * strainFas.at(i)) /
               2.;
    ...
    for (int i = 0; i < _nf; ++i) {
      ...
      // Use a slightly different formulation from Assimaki and Kausel to
      // provide a smooth taper
...
auto operator<<(QDataStream &out, const FrequencyDependentCalculator *fdc)
    -> QDataStream & {
  out << (quint8)2;
...
auto operator>>(QDataStream &in, FrequencyDependentCalculator *fdc)
    -> QDataStream & {
  quint8 ver;
  in >> ver;
