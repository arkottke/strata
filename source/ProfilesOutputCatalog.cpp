...
auto ProfilesOutputCatalog::columnCount(const QModelIndex &parent) const
    -> int {
  Q_UNUSED(parent);
...
auto ProfilesOutputCatalog::data(const QModelIndex &index, int role) const
    -> QVariant {
...
auto ProfilesOutputCatalog::setData(const QModelIndex &index,
...
auto ProfilesOutputCatalog::headerData(int section, Qt::Orientation orientation,
...
auto ProfilesOutputCatalog::flags(const QModelIndex &index) const
    -> Qt::ItemFlags {
  Q_UNUSED(index);
  ...
  // Remove the Enabled flag for RVT motions if the output is for time series
  // only.
...
auto ProfilesOutputCatalog::removeRows(int row, int count,
...
auto operator<<(QDataStream &out, const ProfilesOutputCatalog *poc)
    -> QDataStream & {
  out << (quint8)4;
