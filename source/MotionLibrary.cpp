...
MotionLibrary::MotionLibrary(QObject *parent) : MyAbstractTableModel(parent) {
...
auto MotionLibrary::approachList() -> QStringList {
  return QStringList() << tr("Time Series") << tr("Random Vibration Theory");
}
...
auto MotionLibrary::saveData() const -> bool { return _saveData; }
    ...
    case TypeColumn:
      if (role == Qt::DisplayRole) {
        ...
        return AbstractMotion::typeList().at(
            (int)_motions.at(index.row())->type());
      } else {
...
auto MotionLibrary::headerData(int section, Qt::Orientation orientation,
...
auto MotionLibrary::setData(const QModelIndex &index, const QVariant &value,
                            int role) -> bool {
...
auto MotionLibrary::removeRows(int row, int count, const QModelIndex &parent)
    -> bool {
  if (!count)
    return false;
