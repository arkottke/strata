...
TimeSeriesMotion::TimeSeriesMotion(QObject *parent) : AbstractMotion(parent) {
  _isLoaded = false;
  _saveData = true;
...
TimeSeriesMotion::TimeSeriesMotion(const QString &fileName, double scale,
                                   AbstractMotion::Type type, bool *successful,
                                   QObject *parent)
...
auto TimeSeriesMotion::fileName() const -> QString { return _fileName; }
...
auto TimeSeriesMotion::freqMax() const -> double { return freqNyquist(); }
...
auto TimeSeriesMotion::pointCount() const -> int { return _pointCount; }
...
auto TimeSeriesMotion::timeStep() const -> double { return _timeStep; }
...
auto TimeSeriesMotion::scale() const -> double { return _scale; }
...
void TimeSeriesMotion::setFormat(int format) { setFormat((Format)format); }

auto TimeSeriesMotion::dataColumn() const -> int { return _dataColumn; }
...
auto TimeSeriesMotion::startLine() const -> int { return _startLine; }
...
auto TimeSeriesMotion::stopLine() const -> int { return _stopLine; }
...
auto TimeSeriesMotion::freq() const -> const QVector<double> & { return _freq; }
...
auto TimeSeriesMotion::timeSeries(MotionType type,
                                  const QVector<std::complex<double>> &tf,
                                  const bool baselineCorrect) const
    -> QVector<double> {
...
auto TimeSeriesMotion::name() const -> QString {
  ...
  return fileInfo.dir().dirName() + QString(QDir::separator()) +
         fileInfo.fileName();
}
...
auto TimeSeriesMotion::load(const QString &fileName, bool defaults,
                            double scale) -> bool {
  _accel.clear();
  ...
  if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
    qWarning() << "Unable to open the time series file:"
  ...
  if (_pointCount && _pointCount != _accel.size()) {
    if (stopLineReached) {
      ...
      qCritical()
          << "Number of points read does not equal specified point count!";
      return false;
    }
  }
...
auto TimeSeriesMotion::data(const QModelIndex &index, int role) const
    -> QVariant {
...
auto TimeSeriesMotion::headerData(int section, Qt::Orientation orientation,
...
void TimeSeriesMotion::setSaveData(bool b) { _saveData = b; }

auto TimeSeriesMotion::saveData() const -> bool { return _saveData; }

auto TimeSeriesMotion::isLoaded() const -> bool { return _isLoaded; }

void TimeSeriesMotion::setIsLoaded(bool isLoaded) { _isLoaded = isLoaded; }
...
auto TimeSeriesMotion::max(const QVector<std::complex<double>> &tf) const
    -> double {
  // Return the maximum value in the time history
  return findMaxAbs(calcTimeSeries(_fourierAcc, tf));
}

auto TimeSeriesMotion::maxVel(const QVector<std::complex<double>> &tf) const
    -> double {
  return findMaxAbs(timeSeries(Velocity, tf, false));
}

auto TimeSeriesMotion::maxDisp(const QVector<std::complex<double>> &tf) const
    -> double {
  return findMaxAbs(timeSeries(Displacement, tf, false));
}

auto TimeSeriesMotion::computeSa(const QVector<double> &period, double damping,
...
auto TimeSeriesMotion::ariasIntensity(
    const QVector<std::complex<double>> &tf) const -> QVector<double> {
...
auto TimeSeriesMotion::integrate(const QVector<double> &in) const
    -> QVector<double> {
...
auto TimeSeriesMotion::baselineFit(const int term,
                                   const QVector<double> &series) const
  ...
  gsl_multifit_linear_workspace *work =
      gsl_multifit_linear_alloc(series.size(), term);
...
auto TimeSeriesMotion::absFourier(const QVector<std::complex<double>> &fa,
  ...
  if (!tf.isEmpty()) {
    ...
    // The FAS is scaled by the time-step when it is compared with another
    // motion
    for (int i = 0; i < fa.size(); ++i)
...
void TimeSeriesMotion::fft(const QVector<double> &in,
                           QVector<std::complex<double>> &out) const {
  /*
      ...
      int n = in.size() / 2 + 1;
      fftw_complex* outArray = (fftw_complex*)fftw_malloc( sizeof(fftw_complex)
     * n);

      // Create the plan and execute the FFT
      fftw_plan p = fftw_plan_dft_r2c_1d( in.size(), inArray, outArray,
...
void TimeSeriesMotion::ifft(const QVector<std::complex<double>> &in,
                            QVector<double> &out) const {
  /*
  #ifdef USE_IFFTW
      // Copy the input QVector into a double array
      fftw_complex* inArray = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) *
  in.size());
...
auto TimeSeriesMotion::calcTimeSeries(
    QVector<std::complex<double>> fa,
    const QVector<std::complex<double>> &tf) const -> QVector<double> {
  // Apply the transfer fucntion
  if (!tf.isEmpty()) {
    // If needed, zero pad the Fourier amplitudes such that is has the same
    // length as the incoming transfer function.
    if (fa.size() < tf.size())
      fa.resize(tf.size());
...
auto operator<<(QDataStream &out, const TimeSeriesMotion *tsm)
    -> QDataStream & {
  out << (quint8)3;
...
auto operator>>(QDataStream &in, TimeSeriesMotion *tsm) -> QDataStream & {
  ...
  in >> tsm->_fileName >> tsm->_timeStep >> tsm->_pointCount >> tsm->_scale >>
      format >> tsm->_dataColumn >> tsm->_startLine >> tsm->_stopLine;
