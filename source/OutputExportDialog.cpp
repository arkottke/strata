...
OutputExportDialog::OutputExportDialog(OutputCatalog *model, QWidget *parent,
                                       Qt::WindowFlags f)
...
void OutputExportDialog::createDialog() {
  ...
  _destDirLineEdit->setText(settings
                                .value("outputExportDialog/path",
  ...
  for (int i = 0; i < _model->outputs().size(); ++i) {
    ...
    item->setCheckState(ao->exportEnabled() ? Qt::Checked : Qt::Unchecked);
