...
CompatibleRvtMotion::~CompatibleRvtMotion() { _targetRespSpec->deleteLater(); }

auto CompatibleRvtMotion::freq() const -> const QVector<double> & {
  return _freq->data();
}

auto CompatibleRvtMotion::freqDimension() -> Dimension * { return _freq; }

auto CompatibleRvtMotion::targetRespSpec() -> ResponseSpectrum * {
  return _targetRespSpec;
}

auto CompatibleRvtMotion::limitFas() const -> bool { return _limitFas; }
...
auto CompatibleRvtMotion::loadFromTextStream(QTextStream &stream, double scale)
    -> bool {
  ...
  while (!line.isNull()) {
    ...
    if (!ok) {
      qWarning() << tr("Unable to parse the spectral acceleration in line:")
                 << line;
      return false;
    } else {
      sa << (scale * d);
...
void CompatibleRvtMotion::calculate() {
  ...
  gsl_spline *spline =
      gsl_spline_alloc(gsl_interp_cspline, _targetRespSpec->period().size());

  gsl_spline_init(spline, _targetRespSpec->period().data(), estimateFas.data(),
                  _targetRespSpec->period().size());
  ...
  _respSpec->setSa(
      computeSa(_targetRespSpec->period(), _targetRespSpec->damping()));
...
auto CompatibleRvtMotion::vanmarckeInversion() const -> QVector<double> {
  if (_targetRespSpec->sa().size() < 10) {
    ...
    QVector<double> period = Dimension::logSpace(
        targetPeriod.first(), targetPeriod.last(), int(10 * decades));

    QVector<double> sa;
    logLogInterp(_targetRespSpec->period(), _targetRespSpec->sa(), period, sa);
  ...
  for (int i = fas.size() - 1; i > -1; --i) {
    ...
    if (i == fas.size() - 1) {
      ...
      sum += (fasSqr - prevFasSqr) / 2 *
             (freq - (1 / _targetRespSpec->period().at(i + 1)));
    }
    prevFasSqr = fasSqr;
  }
...
auto operator<<(QDataStream &out, const CompatibleRvtMotion *crm)
    -> QDataStream & {
  out << (quint8)1;
