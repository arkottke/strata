...
AbstractMotion::AbstractMotion(QObject *parent) : MyAbstractTableModel(parent) {
  _enabled = true;
  _type = Outcrop;
...
auto AbstractMotion::typeList() -> QStringList {
  ...
  return QStringList() << tr("Outcrop (2A)") << tr("Within (A+B)")
                       << tr("Incoming Only (A)");
#else
  return QStringList() << tr("Outcrop") << tr("Within");
#endif
}

auto AbstractMotion::variantToType(QVariant variant, bool *ok)
    -> AbstractMotion::Type {
  *ok = false;
  Type type = Outcrop;
...
auto AbstractMotion::modified() const -> bool { return _modified; }
...
auto AbstractMotion::type() const -> AbstractMotion::Type { return _type; }
...
auto AbstractMotion::description() const -> QString { return _description; }
...
auto AbstractMotion::enabled() const -> bool { return _enabled; }

void AbstractMotion::setEnabled(bool enabled) { _enabled = enabled; }

auto AbstractMotion::respSpec() -> ResponseSpectrum * { return _respSpec; }

auto AbstractMotion::pga() const -> double { return _pga; }

auto AbstractMotion::pgv() const -> double { return _pgv; }

auto AbstractMotion::freqMin() const -> double { return freq().first(); }

auto AbstractMotion::freqCount() const -> int { return freq().size(); }

auto AbstractMotion::angFreqAt(int i) const -> double {
  return 2 * M_PI * freqAt(i);
}

auto AbstractMotion::freqAt(int i) const -> double { return freq().at(i); }
...
auto AbstractMotion::calcSdofTf(const double period, const double damping) const
  ...
  for (int i = 0; i < freqCount(); i++) {
    ...
    //
    tf[i] = (-fn * fn) /
            std::complex<double>(freqAt(i) * freqAt(i) - fn * fn,
                                 -2 * (damping / 100) * fn * freqAt(i));
  }
...
auto operator>>(QDataStream &in, AbstractMotion *m) -> QDataStream & {
  ...
  int type;
  in >> type >> m->_description >> m->_enabled;
