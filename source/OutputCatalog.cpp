...
auto OutputCatalog::data(const QModelIndex &index, int role) const -> QVariant {
  ...
  if (role == Qt::DisplayRole || role == Qt::EditRole) {
    ...
    if (site < _enabled.size() && motion < _enabled.at(site).size())
      return _enabled.at(site).at(motion) ? Qt::Checked : Qt::Unchecked;
  }

  return QVariant();
}

auto OutputCatalog::setData(const QModelIndex &index, const QVariant &value,
                            int role) -> bool {
  if (index.parent() != QModelIndex())
    return false;
...
auto OutputCatalog::headerData(int section, Qt::Orientation orientation,
...
auto OutputCatalog::title() const -> const QString & { return _title; }
...
auto OutputCatalog::log() -> TextLog * { return _log; }

auto OutputCatalog::depth() const -> const QVector<double> & { return _depth; }

auto OutputCatalog::time(int motion) const -> const QVector<double> & {
  return _time.at(motion);
}

auto OutputCatalog::frequency() -> Dimension * { return _frequency; }

auto OutputCatalog::period() -> Dimension * { return _period; }

auto OutputCatalog::damping() const -> double { return _damping; }
...
auto OutputCatalog::motionCount() const -> int { return _motionCount; }
...
void OutputCatalog::initialize(int siteCount, MotionLibrary *motionLibrary) {
  ...
  for (int i = 0; i < motionLibrary->rowCount(); ++i) {
    if (motionLibrary->motionAt(i)->enabled()) {
      ...
      if (auto *tsm =
              qobject_cast<TimeSeriesMotion *>(motionLibrary->motionAt(i)))
        _time << tsm->time();
    }
  }
...
void OutputCatalog::saveResults(int motion,
                                AbstractCalculator *const calculator) {
...
auto OutputCatalog::timesAreNeeded() const -> bool { return _timesAreNeeded; }
...
auto OutputCatalog::periodIsNeeded() const -> bool { return _periodIsNeeded; }
...
auto operator<<(QDataStream &out, const OutputCatalog *oc) -> QDataStream & {
  ...
  out << oc->_title << oc->_filePrefix << oc->_enabled << oc->_frequency
      << oc->_frequencyIsNeeded << oc->_period << oc->_periodIsNeeded
      << oc->_damping << oc->_profilesOutputCatalog << oc->_ratiosOutputCatalog
      << oc->_soilTypesOutputCatalog << oc->_spectraOutputCatalog
      << oc->_timeSeriesOutputCatalog << oc->_log
