  ...
  if (role == Qt::DisplayRole || role == Qt::EditRole) {
    ...
    case ThicknessColumn:
      if (index.row() < (rowCount() - 1)) {
        // Soil layers
        return QString::number(_soilLayers.at(index.row())->thickness(), 'f',
                               2);
      } else {
        // Rock layer
    ...
    case VariedColumn:
      return velocityLayer(index.row())->isVaried() ? Qt::Checked
                                                    : Qt::Unchecked;
    }
  }
...
auto SoilProfile::setData(const QModelIndex &index, const QVariant &value,
                          int role) -> bool {
...
auto SoilProfile::headerData(int section, Qt::Orientation orientation,
...
auto SoilProfile::insertRows(int row, int count, const QModelIndex &parent)
    -> bool {
  if (!count)
    return false;
...
auto SoilProfile::removeRows(int row, int count, const QModelIndex &parent)
    -> bool {
  if (!count)
    return false;
...
auto SoilProfile::profileCount() const -> int { return _profileCount; }
...
auto SoilProfile::onlyConverged() const -> bool { return _onlyConverged; }
...
auto SoilProfile::isVaried() const -> bool { return _isVaried; }
...
auto SoilProfile::soilLayers() -> QList<SoilLayer *> & { return _soilLayers; }

auto SoilProfile::subLayers() -> QList<SubLayer> & { return _subLayers; }

auto SoilProfile::bedrock() -> RockLayer * { return _bedrock; }

auto SoilProfile::inputDepth() const -> double { return _inputDepth; }
...
auto SoilProfile::nonlinearPropertyRandomizer()
...
auto SoilProfile::waterTableDepth() const -> double { return _waterTableDepth; }
...
auto SoilProfile::maxFreq() const -> double { return _maxFreq; }
...
auto SoilProfile::waveFraction() const -> double { return _waveFraction; }
...
void SoilProfile::createSubLayers(TextLog *textLog) {
  ...
  if (_profileRandomizer->bedrockDepthVariation()->enabled()) {
    ...
    if (depthToBedrock < minDepthToBedrock) {
      ...
      textLog->append(
          QString("\tDepth to bedrock limited to be greater than 1 %1")
    ...
    if (_profileRandomizer->bedrockDepthVariation()->enabled()) {
      foreach (SoilLayer *layer, _soilLayers) {
        if (layer->depthToBase() > depthToBedrock) {
          ...
          // Truncate the last SoilLayer once the depth to the bedrock is
          // exceeded
      ...
      if (soilLayers.last()->depthToBase() < depthToBedrock) {
        ...
        soilLayers.last()->setThickness(depthToBedrock -
                                        soilLayers.last()->depth());
      }
    } else {
  ...
  if (_disableAutoDiscretization) {
    foreach (SoilLayer *sl, soilLayers) {
      _subLayers << SubLayer(sl->thickness(), depth, vTotalStress,
                             _waterTableDepth, sl);
    ...
    foreach (SoilLayer *sl, soilLayers) {
      ...
      for (int j = 0; j < numSubLayers; ++j) {
        _subLayers << SubLayer(subThickness, depth, vTotalStress,
                               _waterTableDepth, sl);
        // Increment the depth by the subThicknees
        depth += subThickness;
        // Compute the stress at the base of the layer and this to the total
        // stress
...
auto SoilProfile::subLayerCount() const -> int { return _subLayers.size(); }
...
auto SoilProfile::stressReducCoeffProfile(const double pga) const
    -> QVector<double> {
...
auto SoilProfile::subLayerTable() const -> QString {
  ...
  for (int i = 0; i < _subLayers.size(); ++i)
    html += QString("<tr><td>%1<td>%2<td>%3<td>%4<td>%5<td>%6<td>%7<td>%8<td>%"
                    "9<td>%10<td>%11<td>%12<td>%13</tr>")
                .arg(i + 1)
...
auto SoilProfile::toHtml() const -> QString {
  ...
  html += QString(tr("<li><a name=\"Bedrock\">Bedrock<a>"
                     "<table border=\"0\">"
  ...
  if (_isVaried)
    html += QString("<tr><th>Varied:</th><td>%1</td></tr>")
  ...
  if (_profileRandomizer->velocityVariation()->enabled())
    html += QString(tr("<th>Minimum Vs. (%1)</th>"
                       "<th>Maximum Vs. (%1)</th>"
                       "<th>Varied</th>"))
                .arg(Units::instance()->vel());
  ...
  // Table information
  for (int i = 0; i < _soilLayers.size(); ++i) {
    html += QString("<tr><td>%1</td><td>%2</td><td><a "
                    "href=\"#%3\">%3</a></td><td>%4</td>")
  ...
  // Bedrock layer
  html += QString("<tr><td>%1</td><td>---</td><td><a "
                  "href=\"#Bedrock\">Bedrock</a></td><td>%2</td>")
              .arg(_bedrock->depth())
              .arg(_bedrock->avg());
...
void SoilProfile::updateDepths() {
  if (_soilLayers.size()) {
    ...
    for (int i = 0; i < _soilLayers.size() - 1; ++i)
      _soilLayers[i + 1]->setDepth(_soilLayers.at(i)->depth() +
                                   _soilLayers.at(i)->thickness());

    // Update the depth of the rock layer
    _bedrock->setDepth(_soilLayers.last()->depth() +
                       _soilLayers.last()->thickness());
...
auto SoilProfile::createRepresentativeSoilLayer(double top, double base)
    -> SoilLayer * {
...
void SoilProfile::fromJson(const QJsonObject &json) {
  ...
  _nonlinearPropertyRandomizer->fromJson(
      json["nonlinearPropertyRandomizer"].toObject());
...
auto SoilProfile::toJson() const -> QJsonObject {
  ...
  for (const SoilLayer *sl : _soilLayers) {
    ...
    sljo["soilType"] =
        sl->soilType() ? _soilTypeCatalog->rowOf(sl->soilType()) : -1;
    soilLayers << sljo;
  }
...
auto operator<<(QDataStream &out, const SoilProfile *sp) -> QDataStream & {
  ...
  out << sp->_bedrock << sp->_profileRandomizer
      << sp->_nonlinearPropertyRandomizer << sp->_inputDepth << sp->_isVaried
      << sp->_profileCount << sp->_maxFreq << sp->_waveFraction
      << sp->_disableAutoDiscretization << sp->_waterTableDepth
      << sp->_onlyConverged;
...
auto operator>>(QDataStream &in, SoilProfile *sp) -> QDataStream & {
  ...
  while (sp->_soilLayers.size() < count) {
    ...
    in >> sl >> row;
    // If no soil type is defined for the soil layer set it the pointer to be
    // zero
    sl->setSoilType(row < 0 ? nullptr : sp->_soilTypeCatalog->soilType(row));
  ...
  in >> sp->_bedrock >> sp->_profileRandomizer >>
      sp->_nonlinearPropertyRandomizer >> sp->_inputDepth >> sp->_isVaried >>
      sp->_profileCount >> sp->_maxFreq >> sp->_waveFraction;
