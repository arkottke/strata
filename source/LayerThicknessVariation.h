...
/*! Class for generating layer thicknesses from a non-homogeneous Poisson
 * process. The layering of the profile is computed from a Poisson process with
...
class LayerThicknessVariation : public QObject {
  ...
  friend auto operator<<(QDataStream &out, const LayerThicknessVariation *ltv)
      -> QDataStream &;
  friend auto operator>>(QDataStream &in, LayerThicknessVariation *ltv)
      -> QDataStream &;

public:
  explicit LayerThicknessVariation(gsl_rng *rng,
