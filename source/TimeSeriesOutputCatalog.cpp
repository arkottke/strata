...
TimeSeriesOutputCatalog::TimeSeriesOutputCatalog(OutputCatalog *outputCatalog)
  ...
  _lookup["Visco-Elastic Shear-Stress Time Series"] =
      "ViscoElasticStressTimeSeriesOutput";
#endif
}
...
auto TimeSeriesOutputCatalog::columnCount(const QModelIndex &parent) const
    -> int {
  Q_UNUSED(parent);
  return 4;
}

auto TimeSeriesOutputCatalog::data(const QModelIndex &index, int role) const
    -> QVariant {
  ...
  if (role == Qt::DisplayRole || role == Qt::EditRole) {
    ...
    case TypeColumn:
      if (role == Qt::DisplayRole) {
        return AbstractMotion::typeList().at(_outputs.at(index.row())->type());
      } else {
    ...
    case CorrectedColumn:
      return _outputs.at(index.row())->baselineCorrect() ? Qt::Checked
                                                         : Qt::Unchecked;
    default:
      return QVariant();
    }
...
auto TimeSeriesOutputCatalog::setData(const QModelIndex &index,
...
auto TimeSeriesOutputCatalog::headerData(int section,
...
auto TimeSeriesOutputCatalog::flags(const QModelIndex &index) const
    -> Qt::ItemFlags {
  ...
  case DepthColumn:
    ...
    break;
  case TypeColumn:
    if (qobject_cast<const StrainTimeSeriesOutput *const>(
            _outputs.at(index.row())) ||
        qobject_cast<const StressTimeSeriesOutput *const>(
            _outputs.at(index.row())) ||
        qobject_cast<const ViscoElasticStressTimeSeriesOutput *const>(
            _outputs.at(index.row()))) {
...
auto TimeSeriesOutputCatalog::removeRows(int row, int count,
...
auto operator<<(QDataStream &out, const TimeSeriesOutputCatalog *tsoc)
    -> QDataStream & {
  out << (quint8)1;
...
auto operator>>(QDataStream &in, TimeSeriesOutputCatalog *tsoc)
    -> QDataStream & {
  quint8 ver;
  in >> ver;
