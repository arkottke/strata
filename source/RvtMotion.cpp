...
auto RvtMotion::setData(const QModelIndex &index, const QVariant &value,
                        int role) -> bool {
...
auto RvtMotion::insertRows(int row, int count, const QModelIndex &parent)
    -> bool {
  if (!count)
    return false;
...
auto RvtMotion::removeRows(int row, int count, const QModelIndex &parent)
    -> bool {
  if (!count)
    return false;
  ...
  for (int i = 0; i < logXi.size(); ++i) {
    for (int j = 0; j < logX.size() - 1; ++j) {
      if (abs(logXi.at(i) - logX.at(j)) < 0.000001) {
        ...
        double slope =
            (logY.at(j + 1) - logY.at(j)) / (logX.at(j + 1) - logX.at(j));
        ...
        // Extrapolate based on the first two values
        double slope =
            (logY.at(1) - logY.first()) / (logX.at(1) - logX.first());
 ...
 * \param window the number of ponits on either side of a given point to average
 * against
 */
...
auto RvtMotion::freq() const -> const QVector<double> & { return _freq; }
...
auto RvtMotion::toHtml() const -> QString {
  ...
  //            html += tr("</table><table><tr><th>Frequency (Hz)</th><th>FAS
  //            (g-s)</th></tr>");
  //
  ...
  //
  //            html += tr("</table><table><tr><th>Period (s)</th><th>Spectral
  //            Accel. (g)</th></tr>");
  //
