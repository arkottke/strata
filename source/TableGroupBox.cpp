...
auto TableGroupBox::lastRowFixed() const -> bool { return _lastRowFixed; }
...
auto TableGroupBox::table() -> MyTableView * { return _table; }
...
void TableGroupBox::addRow() {
  ...
  if (selectedRows.isEmpty()) {
    ...
    _table->model()->insertRows(_table->model()->rowCount(),
                                selectedRows.size());
  }
...
void TableGroupBox::cellSelected() // const QModelIndex & /* current */, const
                                   // QModelIndex & /*previous*/)
{
  ...
  if (!selectedRows.isEmpty()) {
    ...
    if (_lastRowFixed &&
        selectedRows.last().row() == _table->model()->rowCount() - 1)
      _removeButton->setEnabled(false);
    else
