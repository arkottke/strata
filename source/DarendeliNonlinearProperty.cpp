...
DarendeliNonlinearProperty::DarendeliNonlinearProperty(Type type,
                                                       QObject *parent)
...
void DarendeliNonlinearProperty::calculate(const SoilType *soilType) {
  ...
  double refStrain =
      (0.0352 + 0.0010 * soilType->pi() * pow(soilType->ocr(), 0.3246)) *
      pow(soilType->meanStress(), 0.3483);
  ...
  for (int i = 0; i < _strain.size(); ++i) {
    ...
    if (_type == ModulusReduction) {
      ...
      const double minDamping =
          (0.8005 + 0.0129 * soilType->pi() * pow(soilType->ocr(), -0.1069)) *
          pow(soilType->meanStress(), -0.2889) *
          (1 + 0.2919 * log(soilType->freq()));

      // Masing damping based on shear-modulus reduction
      const double masingDamping_a1 =
          (100. / M_PI) *
          (4 *
               (_strain.at(i) -
                refStrain * log((_strain.at(i) + refStrain) / refStrain)) /
               (pow(_strain.at(i), 2.) / (_strain.at(i) + refStrain)) -
           2.);
      ...
      // double masingD = c1 * masingD_a1 + c2 * pow( masingD_a1, 2. ) + c3 *
      // pow( masingD_a1, 3. );
      const double masingDamping = c1 * masingDamping_a1 +
                                   c2 * pow(masingDamping_a1, 2.) +
                                   c3 * pow(masingDamping_a1, 3.);
