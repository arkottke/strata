...
class AbstractMotion : public MyAbstractTableModel {
  ...
  friend auto operator<<(QDataStream &out, const AbstractMotion *m)
      -> QDataStream &;
...
public:
  ...
  //! Type of motion
  enum Type {
    Outcrop,     //!< AbstractMotion has free surface amplification and contains
                 //!< both incoming (A) and reflected (B) waves are identical
  ...
  virtual auto max(const QVector<std::complex<double>> &tf =
                       QVector<std::complex<double>>()) const -> double = 0;
  ...
  //! The absolute value of the acceleration Fourier amplitude spectrum with the
  //! applied transfer function
  virtual auto absFourierAcc(const QVector<std::complex<double>> &tf =
                                 QVector<std::complex<double>>()) const
      -> const QVector<double> = 0;

  //! The absolute value of the velocity Fourier amplitude spectrum with the
  //! applied transfer function
...
protected:
   ...
   */
  auto calcSdofTf(const double period, double damping) const
      -> QVector<std::complex<double>>;
