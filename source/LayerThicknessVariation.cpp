...
auto LayerThicknessVariation::modelList() -> QStringList {
  ...
  list << tr("Custom") << tr("Default (Toro 1995)");
...
void LayerThicknessVariation::setModel(int model) { setModel((Model)model); }

auto LayerThicknessVariation::customEnabled() const -> bool {
  return _model == Custom;
}

auto LayerThicknessVariation::coeff() const -> double { return _coeff; }
...
auto LayerThicknessVariation::initial() const -> double { return _initial; }
...
auto LayerThicknessVariation::exponent() const -> double { return _exponent; }
...
auto LayerThicknessVariation::vary(double depthToBedrock) const
    -> QList<double> {
  ...
  while (prevDepth < depthToBedrock) {
    ...
    double depth = pow((_exponent * sum) / _coeff + sum / _coeff +
  ...
  // Correct the last layer of thickness so that the total depth is equal to the
  // maximum depth
  if (thicknesses.size())
...
auto operator<<(QDataStream &out, const LayerThicknessVariation *ltv)
    -> QDataStream & {
  out << (quint8)1;
...
auto operator>>(QDataStream &in, LayerThicknessVariation *ltv)
    -> QDataStream & {
  quint8 ver;
  in >> ver;
  ...
  ltv->setModel(model);
  if (ltv->_model == LayerThicknessVariation::Custom) {
    in >> ltv->_coeff >> ltv->_initial >> ltv->_exponent;
  }
