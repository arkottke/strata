...
VelocityVariation::VelocityVariation(gsl_rng *rng,
                                     ProfileRandomizer *profileRandomizer)
    : _enabled(false), _stdevModel(Custom), _stdevIsLayerSpecific(false),
...
auto VelocityVariation::modelList() -> QStringList {
  ...
  list << tr("Custom") << tr("GeoMatrix A&B, Rock & Shallow Soil")
       << tr("GeoMatrix C&D, Deep Narrow/Broad Soil");

  if (Units::instance()->system() == Units::Metric) {
    list << tr("USGS A&B, >360 m/s") << tr("USGS C&D, <360 m/s")
         << tr("USGS A, >750 m/s") << tr("USGS B, 360-750 m/s")
         << tr("USGS C, 180 to 360 m/s") << tr("USGS D, <180 m/s");
  } else {
    list << tr("USGS A&B, >1200 ft/s") << tr("USGS C&D, <1200 ft/s")
         << tr("USGS A, >2500 ft/s") << tr("USGS B, 1200-2500 ft/s")
         << tr("USGS C, 600 to 1200 ft/s") << tr("USGS D, <600 ft/s");
  }
...
auto VelocityVariation::stdev() const -> double { return _stdev; }
...
auto VelocityVariation::correlFinal() const -> double { return _correlFinal; }
...
auto VelocityVariation::correlDelta() const -> double { return _correlDelta; }
...
void VelocityVariation::vary(QList<SoilLayer *> &soilLayers,
                             RockLayer *bedrock) const {
  ...
  for (int i = 0; i < soilLayers.size(); ++i) {
    ...
    double depthToMid =
        soilLayers.at(i)->depth() + soilLayers.at(i)->thickness() / 2.;
    ...
    if (i == 0) {
      ...
      const double dCorrel =
          (depthToMid <= 200)
              ? _correlFinal *
                    pow((depthToMid + _correlInitial) / (200 + _correlInitial),
                        _correlExponent)
              : _correlFinal;
      // Thickness dependent correlation. Again convert to meters
      thickness = depthToMid - prevDepthToMid;
      const double tCorrel =
          _correlInitial * exp(-toMeters * thickness / _correlDelta);
  ...
  if (bedrock->isVaried()) {
    ...
    bedrock->setVaried(
        qMax(bedrock->avg() * exp(randVar), soilLayers.last()->shearVel()));
...
auto operator<<(QDataStream &out, const VelocityVariation *vv)
    -> QDataStream & {
  out << (quint8)1;
  ...
  if (vv->_correlModel == VelocityVariation::Custom) {
    out << vv->_correlInitial << vv->_correlFinal << vv->_correlDelta
        << vv->_correlIntercept << vv->_correlExponent;
  }
...
auto operator>>(QDataStream &in, VelocityVariation *vv) -> QDataStream & {
  ...
  if (vv->_stdevModel == VelocityVariation::Custom) {
    in >> vv->_stdevIsLayerSpecific >> vv->_stdev;
  }
  ...
  if (vv->_correlModel == VelocityVariation::Custom) {
    in >> vv->_correlInitial >> vv->_correlFinal >> vv->_correlDelta >>
        vv->_correlIntercept >> vv->_correlExponent;
  }
