...
CrustalModel::CrustalModel(QObject *parent) : MyAbstractTableModel(parent) {}
...
auto CrustalModel::headerData(int section, Qt::Orientation orientation,
...
auto CrustalModel::setData(const QModelIndex &index, const QVariant &value,
                           int role) -> bool {
...
auto CrustalModel::calculate(const QVector<double> &freq) const
    -> QVector<double> {
  ...
  for (int i = 0; i < freq.size(); ++i) {
    ...
    amp[i] =
        sqrt((_velocity.at(i) * _density.at(i)) / (avgDensity / avgSlow.at(i)));
  }
