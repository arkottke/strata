...
NonlinearPropertyRandomizer::NonlinearPropertyRandomizer(
    gsl_rng *rng, SoilProfile *siteProfile)
...
auto NonlinearPropertyRandomizer::model() const
    -> NonlinearPropertyRandomizer::Model {
  return _model;
}
...
auto NonlinearPropertyRandomizer::correl() const -> double { return _correl; }
...
auto NonlinearPropertyRandomizer::dampingUncert()
    -> NonlinearPropertyUncertainty * {
  return _dampingUncert;
}

auto NonlinearPropertyRandomizer::modulusUncert()
    -> NonlinearPropertyUncertainty * {
...
void NonlinearPropertyRandomizer::vary(RockLayer *bedrock) {
  bedrock->setDamping(_dampingUncert->variedDamping(
      _model, bedrock->avgDamping(), gsl_ran_gaussian(_rng, 1)));
}
...
auto operator<<(QDataStream &out, const NonlinearPropertyRandomizer *npv)
    -> QDataStream & {
  out << (quint8)2;
...
auto operator>>(QDataStream &in, NonlinearPropertyRandomizer *npv)
    -> QDataStream & {
  quint8 ver;
  in >> ver;
  ...
  if (ver < 2) {
    // Read information previously stored in the
    // NonlinearPropertyStandardDeviation
    quint8 ui;
    double d;
    QString s;
    for (int i = 0; i < 2; ++i) {
