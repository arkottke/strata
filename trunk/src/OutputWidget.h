...
class OutputWidget : public QSplitter
{
    Q_OBJECT
	
    public:
        OutputWidget( SiteResponseOutput * model = 0, QWidget * parent = 0);
        ~OutputWidget();

        enum Source{
            Input, //!< Model comes from the input widget and should not be deleted
            File //!< Model comes from a file and should be deleted
        };

        SiteResponseOutput * model() const;
        void setModel( SiteResponseOutput * model, Source source);
