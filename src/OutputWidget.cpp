...
OutputWidget::OutputWidget( SiteResponseOutput * model, QWidget * parent)
...
SiteResponseOutput * OutputWidget::model() const
{
    return m_model;
}
        
void OutputWidget::setModel( SiteResponseOutput * model, OutputWidget::Source source)
{
    if ( m_model != model ) {
        if (m_model)
        ...
        // Reform connections
        connect( m_model, SIGNAL(dataChanged()), m_plot, SLOT(replot()));
...
void OutputWidget::setStatsNeedUpdate()
{
    m_statsNeedUpdate = true;
    m_recomputePushButton->setEnabled(true);
}

void OutputWidget::open(const QString fileName)
{
    SiteResponseOutput * model = new SiteResponseOutput;
...
void OutputWidget::save(const QString fileName)
{
    // Save the file
    m_model->save(fileName);
}

void OutputWidget::exportData()
{
...
void OutputWidget::print(QPrinter * printer)
{
...
void OutputWidget::updateWidget()
{
    m_outputComboBox->clear();
    m_outputComboBox->addItems(m_model->outputNames());

    m_tableModel->setModel(m_model);
    m_tableView->resizeColumnsToContents();
}

void OutputWidget::showPlotContextMenu(const QPoint & point)
{
    m_plotContextMenu->popup( m_plot->mapToGlobal(point) );
}

void OutputWidget::copyPlot()
{
    uncolorSelectedCurve();
...
void OutputWidget::configurePlot()
{
...
void OutputWidget::selectedChanged( const QModelIndex & current, const QModelIndex & /*previous*/ )
{
...
void OutputWidget::plotOutput(int index)
{
...
void OutputWidget::pointSelected(const QPoint & point)
{
...
void OutputWidget::enableSelectedCurve()
{
    colorSelectedCurve();
...
void OutputWidget::colorSelectedCurve()
{
...
void OutputWidget::uncolorSelectedCurve()
{
    if ( !m_selectedOutput || m_selectedRow < 0 )
        return;
...
void OutputWidget::setMotionEnabled(bool enabled)
{
    if (  0 <= m_selectedRow )
        m_model->setMotionEnabledAt( m_selectedRow, enabled);
    
    m_enableMotionPushButton->setText( QString(tr("%1 Motion: %2"))
            .arg( enabled ? tr("Enable") : tr("Disable") )
            .arg( m_model->motionNameAt(m_selectedRow) ) );
...
void OutputWidget::setSiteEnabled(bool enabled)
{
    if (  0 <= m_selectedRow )
...
void OutputWidget::recomputeStats()
{
...
void OutputWidget::populateQuantiles()
{
...
void OutputWidget::createTableGroup() 
{
...
void OutputWidget::createContextMenu() 
{
    // Create the context menu
    m_plotContextMenu = new QMenu;
    m_plotContextMenu->addAction(tr("Copy"), this, SLOT(copyPlot()));
